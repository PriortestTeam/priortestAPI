...
RUN /bin/sh -c 'java -Xscmx50M -Xshareclasses -Xquickstart -jar -Dspring.profiles.active=test oneclick.jar &' ;
RUN sleep 25 ;
RUN ps aux | grep java | grep petclinic | awk '{print $1}' | xargs kill -1
