...
-- 为字段添加索引以提升查询性能
CREATE INDEX idx_issue_found_version ON `issue` (`found_version`);
...
-- 组合索引用于常见的组合查询
CREATE INDEX idx_issue_version_analysis ON `issue` (`found_version`, `is_legacy`, `found_after_release`);
