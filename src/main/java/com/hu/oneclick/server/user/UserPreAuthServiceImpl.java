...
public class UserPreAuthServiceImpl implements UserPreAuthService {
    ...
    public Resp<String> register(RegisterBody registerBody) {
        ...
        if (!sysUsers.isEmpty()) {
            return new Resp.Builder<String>().buildResult(SysConstantEnum.ALREADYHAS_EMAIL.getCode(), SysConstantEnum.ALREADYHAS_EMAIL.getValue());
        }
        
        // 查询公司房间是否存在
        Room room = roomDao.queryByCompanyNameAndEmail(registerBody.getCompany(), registerBody.getEmail());
        if (room == null) {
            // 创建新房间
            room = new Room();
        ...
        if (result > 0) {
            ...
            try {
                mailService.sendActivationEmail(registerBody.getEmail(), activationUrl);
                ...
                return new Resp.Builder<String>().buildResult(SysConstantEnum.EMAIL_SEND_FAILED.getCode(), SysConstantEnum.EMAIL_SEND_FAILED.getValue());
            }
        } else {
            return new Resp.Builder<String>().buildResult(SysConstantEnum.DATABASE_ERROR.getCode(), SysConstantEnum.DATABASE_ERROR.getValue());
        }
    }
}
